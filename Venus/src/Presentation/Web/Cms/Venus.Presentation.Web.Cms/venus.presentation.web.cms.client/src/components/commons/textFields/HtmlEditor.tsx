...
export const HtmlEditor = (props:HtmlEditorProps)=>{
    ...
    const editorConfig = {
        ...
        plugins: [
            ...
            ImageUpload, ImageInsert, ImageInsertViaUrl // URL desteği için kritik olanlar
        ],
    
        toolbar: [
            'sourceEditing', // Kaynak kodu
            '|',
            'insertImage',   // URL İLE RESİM EKLEME VE YÜKLEME BUTONU BURADA
            'mediaEmbed',    // Video ekleme
            '|',
            'undo', 'redo',
            '|',
            'heading',
            '|',
            'bold', 'italic', 'underline', 'link', 'insertTable',
            '|',
            'bulletedList', 'numberedList', 'alignment'
        ],
    
        // Resim ayarlarını detaylandırıyoruz:
        image: {
            // Resim eklendiğinde hangi metodların sunulacağını belirler
            insert: {
                type: 'auto' // Hem URL hem de Dosya Yükleme (Upload) arayüzünü açar
            },
            toolbar: [
        ...
        // Diğer ayarlar (htmlSupport vb.) aynı kalacak...
    };

    return (
        <div style={{ padding: '20px' }}>
            <h2>Hızlı Entegrasyon</h2>
            <CKEditor
                ...
                data="<p>Editör hazır!</p>"
                onReady={ editor => {
                    console.log( 'Editör kullanıma hazır!', editor );
                } }
                onChange={ ( event, editor ) => {
                    const data = editor.getData();
                    console.log( 'İçerik değişti:', data );
                } }
            />
        </div>
    );
